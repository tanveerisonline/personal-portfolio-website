...
const cardProjectSection = document.querySelector(".works");
let index = 0;
projectCardInfo.forEach((project) => {
  cardProjectSection.innerHTML += `
          <div class="create-flex work-card">
            <img src="${
              project.cardImg
            }" class="card-work-img" alt="Portfolio Image">
            <div class="work-card-context">
              <h2 class="card-title">${project.cardTitle}</h2>
              <div class="make-grid work-categories">
                
                  
                <p class="canopy grid-item">${project.cardCanopy}</p>
...
for (let i = 0; i < cards.length; i += 1) {
  if (i % 2 === 1) {
    cards[i].classList.add("reverse-img");
  }
}
